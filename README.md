...
> 一个可复用、可下载给 Agent 使用的 **AI Agent Skills**，用于成都大学图书馆馆藏调研、学习路线规划和书籍推荐。

`cdu-library-book-survey` 是一个 **Agent-neutral Skills 仓库**，面向成都大学学生的学习规划与馆藏调研。它不是一次性 Prompt，也不是单纯脚本，而是一套可被 Codex、Claude Code、Cursor、Gemini CLI、ChatGPT Skill 或其他 Agent 下载后使用的 **Skill 规则、执行流程、脚本和报告模板**。
...
### 方式一：把 GitHub 链接发给 Agent

用户可以直接把本仓库链接发给支持下载仓库的 Agent，让 Agent 安装或读取这个 skills 后执行：

```text
请下载并使用这个 skills：
https://github.com/A7m0spHere/cdu-library-book-survey

我需要调研成都大学图书馆馆藏，并根据我的专业、阶段、课程或目标方向推荐书籍。

请输出 normal 深度报告，至少包含：
- 馆藏推荐 Top10
- 学习价值 Top10
- 学习路线映射
- 能力映射
